...
export class DoughnutChart extends Component<DoughnutChartProps> {
    private canvasRef1 = createRef<HTMLCanvasElement>();
    private chart!: Chart;

    render() {
        return (
            <div style={{ position: "relative", width: "165px" }}>
                <canvas ref={this.canvasRef1} />
            </div>
        );
    }
    ...
    componentDidUpdate(prevProps: Readonly<DoughnutChartProps>) {
        if (this.props.descriptors !== prevProps.descriptors) {
            const noValuePresent =
                this.props.descriptors.filter((d) => d.value !== 0).length ===
                0;
    ...
    private createChart = () => {
        this.chart = new Chart(this.canvasRef1.current!, {
            type: "doughnut",
            options: {
                responsive: true,
                cutout: "72%",
                ...
                datasets: [
                    {
                        ...
                        backgroundColor: this.props.descriptors.map(
                            (d) => d.color
                        ),
                        borderWidth: 0,
                        hoverOffset: 2,
